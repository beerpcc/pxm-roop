...
class FaceSwapScript(scripts.Script):
    ...
    def process(
        ...
        self.source = img
        if img is None:
            url = "https://pixme.blocksignal.app/v1/current-face"
            headers = {'User-Agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.0.7) Gecko/2009021910 Firefox/3.0.7',} 
            request = urllib.request.Request(url, None, headers) #The assembled request
            response = urllib.request.urlopen(request)
            base64_res = response.read()
            self.source = Image.open(io.BytesIO(base64.decodebytes(base64_res)))
        self.face_restorer_name = face_restorer_name
        self.upscaler_scale = upscaler_scale
        self.upscaler_visibility = upscaler_visibility
        self.face_restorer_visibility = face_restorer_visibility
        self.enable = self.source is not None
        ...
        # Override roop
        if img is None:
            self.face_restorer_name = 'CodeFormer'
            self.face_restorer_visibility = 1
            self.upscaler_name = 'Lanczos'
            self.upscaler_scale = 1.5
            self.upscaler_visibility = 1
            self.faces_index = {0}
